...
To assign a role for a resource, you need the `mdb.admin` role, `managed-mysql.admin` role, or one of the following roles for that resource:
...
As a user, you need the [{{ roles.mmy.editor }} role or higher](../../iam/concepts/access-control/roles.md) for the folder that will contain the new cluster. The `{{ roles.mmy.viewer }}` role only allows you to view the list of clusters.
